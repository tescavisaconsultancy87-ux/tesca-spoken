...
export default function Trainers() {
  ...
  // Keep a ref in sync so scroll callbacks always see the latest count
  useEffect(() => {
  ...
  const updateScrollState = useCallback(() => {
    ...
    // Determine active index: find the child whose left edge is closest to the container's left edge
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (!child) continue;
      // offsetLeft is relative to container (which is position:relative)
  ...
  useEffect(() => {
    ...
    // Small delay to let the DOM fully render before measuring
